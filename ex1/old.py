...
def main_direct(video_path, video_type) -> tuple:
    # Parameters
    ...
    while True:
        ...
        # Compare the histograms between frames
        if previous_hist is not None:
            # Calculate the distance between histograms
            dist = euclidean(previous_hist, equalized_hist)
...
def main_cumsum_no_equalize(video_path, video_type) -> tuple:
    distance_threshold = threshhold_factory(video_type)  # Set the threshold distance for cumulative histogram difference
    output_frames_dir = 'frames'
    os.makedirs(output_frames_dir, exist_ok=True)
...
def main_direct_no_equazlize(video_path, video_type) -> tuple:
    distance_threshold = threshhold_factory(video_type)  # Set the threshold distance for direct histogram difference
    output_frames_dir = 'frames'
    os.makedirs(output_frames_dir, exist_ok=True)
    ...
    while True:
        ...
        if previous_hist is not None:
            dist = euclidean(previous_hist, hist)
